...
[ "$#" -ge 5 ] || die "Usage ./redshift_ad_reports.sh [game-id] [app-env] [year] [month] [day] [hour] (eg. ./redshift_load_data.sh pogs stage 2013 10 07 15)"
...
DBPORT=5439
DBNAME=tntserenity
DBTABLE=${GAME_ID}_ad_events_${APP_ENV}
DBUSER=tntserenity
DBPASS=TNT4ever!
...
if [ $HOURLY -eq 1 ]; then
	S3PATH_AD_COUNTS_REPORT=s3://${BUCKET}/${GAME_ID}/ads/${YEAR}/${MONTH}/${DAY}/${HOUR}/${AD_EVENTS_COUNT_REPORT_NAME}_
	QUERY_HOUR=$HOUR:00:00
	QUERY_INTERVAL_SECS=3600
else
	S3PATH_AD_COUNTS_REPORT=s3://${BUCKET}/${GAME_ID}/ads/${YEAR}/${MONTH}/${DAY}/${AD_EVENTS_COUNT_REPORT_NAME}_daily_
	QUERY_HOUR=00:00:00
...
# Secure temp files
export PGPASSFILE=`mktemp /tmp/pass.${GAME_ID}.${APP_ENV}.${YEAR}.${MONTH}.${DAY}.${HOUR}.XXXXXXXXXX`
cmds=`mktemp /tmp/cmds.${GAME_ID}.${APP_ENV}.${YEAR}.${MONTH}.${DAY}.${HOUR}.XXXXXXXXXX`
logs=`mktemp /tmp/logs.${GAME_ID}.${APP_ENV}.${YEAR}.${MONTH}.${DAY}.${HOUR}.XXXXXXXXXX`
